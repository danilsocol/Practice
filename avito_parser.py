...
class AvitoParse(Parser):
    def __init__(self, city, user_id): #user
    ...
    def get_ads(self, request, price_from: None, price_to: None):
        ...
        links_on_page = self.driver.find_elements(By.CLASS_NAME, "iva-item-content-rejJg")
        try:
    ...
    @staticmethod
    def parse_card(card_element, user_id, city): #user_id, city
        card = dict()
        card['user_id'] = user_id #user
    ...
    def parse(self, request, price_from, price_to):
        ...
        for el in self.get_ads(request, price_from, price_to):
            mass_of_cards.append(self.parse_card(el, self.user_id, self.city)) #user_id, city
        return mass_of_cards

    def change_price(self, url_ad):
